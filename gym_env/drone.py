...
class Drone:
    def __init__(self, id, env: DroneSim, global_path, area, area_to_grid, drone_model, stub=False, detect_obs=False):
        self.saved_posn = None
        self.next_goal = None
        self.id = id
        self.env = env
        self.global_path = []
        self.clean_global_path(global_path, area_to_grid, area, stub)
    ...
    def clean_global_path(self, global_path, area_to_grid, area, stub):
        if stub:
            for arr in np.array([[500.0, 500.0], [500.0, 600.0], [500.0, 700.0]]):
                self.global_path.append(arr)
        else:
            global_path = self.env.meter_to_world_map((global_path - 0.5) * area_to_grid - area/2)
            for gb in global_path:
                if self.env.occ_map[gb[1], gb[0]] == 0:
                    self.global_path.append(gb)
        if len(self.global_path) == 0:
            self.global_path.append(self.get_curr_posn(xyz=False) + 10)
        return
    ...
    def update(self) -> None:
        ...
        next_global_goal_posn = self.global_path[self.iter]
        ...
        local_occ_map = self.get_local_occmap(self.env.occ_map.copy(), self.get_curr_posn(xyz=False, saved_posn=0),
                                              next_global_goal_posn)
        ...
        self.rrt.update_occmap(self.get_local_occmap(newocc_map, self.get_curr_posn(xyz=False, saved_posn=0),
                                                     next_global_goal_posn).copy())
        plot_rrt = self.rrt.plot_graph()
        if self.env.RECORD and not self.env.GUI:
            cv.imwrite(self.env.ONBOARD_IMG_PATH + "/drone_" + str(self.id) + "/" +
                    f"rrt_drone_{self.id}_iter_{self.iter}.png", transform_occ_img(plot_rrt))
            # threading error
    ...
    def map_update(self) -> None:
        ...
        local_occ_map = self.get_local_occmap(self.env.occ_map.copy(), self.get_curr_posn(xyz=False, saved_posn=1),
                                              next_global_goal_posn)
        ...
        newocc_map = self.env.occ_map_red.copy()
        self.rrt.update_occmap(self.get_local_occmap(newocc_map, self.get_curr_posn(xyz=False, saved_posn=1),
                                                     next_global_goal_posn).copy())
        plot_rrt = self.rrt.plot_graph()
    ...
    def step_action(self, obs, debug=False) -> tuple[Any, bool]:
        ...
        if debug:
            logger.info(
                f"drone {self.id} : current_drone_posn: {curr_pos}, next_micro_goal_posn: {goal_posn}, next_local_goal_posn: {self.local_goal_posn}")

        # if current and global goal position are same, increment the global goal position
        if np.allclose(self.get_curr_posn(xyz=False), self.global_path[self.iter], atol=10):
